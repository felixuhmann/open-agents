...
import {
  ...
  type WorkflowConversationListItem,
} from "@/lib/queries";
...
export default function WorkflowConversationsPage() {
  const { slug } = useParams<{ slug: string }>();
  const workflow = useWorkflow(slug);
  const conversations = useWorkflowConversations(slug);
    ...
    return (
      <Empty>
        <EmptyHeader>
          ...
          <EmptyDescription>
            That slug doesn&apos;t map to a workflow in this deployment.
          </EmptyDescription>
        </EmptyHeader>
      </Empty>
    );
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        ...
        meta={w.slug}
        description={`Your chat history with ${w.displayName}.`}
        actions={
          w.webEnabled ? (
            <Button asChild>
      ...
      {conversations.isLoading ? (
        ...
        <Empty>
          <EmptyHeader>
            ...
            <EmptyDescription>You haven&apos;t run {w.displayName} yet.</EmptyDescription>
          </EmptyHeader>
          {w.webEnabled ? (
            <EmptyContent>
              <Button asChild>
...
function ConversationRow({
  ...
  conversation: WorkflowConversationListItem;
  ...
  return (
    <>
      ...
      <Link
        to={`/workflows/${conversation.workflow.slug}/chat/${conversation.id}`}
