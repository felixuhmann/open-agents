...
Inbound attachments are stored as `EmailAttachment` rows. On the next run, rows without `backendFileId` are uploaded to the agent backend and mounted into the OpenSandbox sandbox under `/workspace/inbox/`.
