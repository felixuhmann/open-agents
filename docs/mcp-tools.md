...
| `platform` | `memory`     | `memory_create`, `memory_read`, `memory_list`, `memory_update`, `memory_delete` | Generic JSON-doc collection store, scoped to the calling agent. The agent's prompt or skill describes which collections it uses. |
